...
```
This regenerates `graph/server/generated.go` and `graph/model/models_gen.go`. Implement any new resolver stubs in `internal/handlers/schema_graph_resolver.go`.
